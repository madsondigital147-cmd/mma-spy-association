...
const Body = z.object({
  ...
  kind: z.enum(["meta", "reviews", "tiktok", "youtube"]).optional(),
});
...
export async function POST(req: Request) {
  ...
  const { niche, markets, keywords, kind } = parsed.data;
  if (parseKeywords(keywords).length === 0) {
    return NextResponse.json({ error: "nenhuma palavra-chave válida" }, { status: 400 });
  }

  const source = await prisma.source.create({
    data: { niche, markets: markets.join(","), keywords, status: "active", kind: kind ?? "meta" },
  });
